...
var indexVue = new _vue2.default({
	el: "#todolist",
	data: {
	...
	methods: {
		searchStorage: function searchStorage() {
			//更新事件
		...
		addList: function addList(event) {
			//增加事件
			if (event.keyCode == 13) {
				if (this.list != "") {
		...
		removelist: function removelist(value) {
			//删除事件
			localStorage.removeItem(value);
			this.searchStorage();
		},
		changelist: function changelist(value, key) {
			//改变事件状态
		...
		blur: function blur(key) {
			//事件失去焦点，将input改成p，更新事件内容
			localStorage[key] = this.changesession[key];
			this.showp[key] = true;
			this.searchStorage();
		},
		clearwarn: function clearwarn() {
			this.warn = false;
		}
	},
	directives: {
